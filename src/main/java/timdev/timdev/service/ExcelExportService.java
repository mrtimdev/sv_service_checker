...
public class ExcelExportService {
    ...
    public ByteArrayInputStream exportToExcel(List<ServiceChecker> serviceCheckers) {
        try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            // Create single sheet
            Sheet sheet = workbook.createSheet("Safety Checklists Report");
            ...
            for (ServiceChecker sc : serviceCheckers) {
                ...
                // Safety Checklists Header (collapsible group)
                    ...
                    for (ServiceCheckerItem item : sc.getItems()) {
                        if (item.getNotes() != null && !item.getNotes().isEmpty()) {
                            ...
                            for (ServiceCheckerItemNote note : item.getNotes()) {
                                ...
                                // Safety Checklists Info (only on first row of this service check)
                                if (rowNum - 1 == startRow) {
                                    ...
                                    inspectorCell.setCellValue(
                                            sc.getCreatedBy() != null ? sc.getCreatedBy().fullName() : "");
                    ...
                    if (sc.getItems() != null) {
                        for (ServiceCheckerItem item : sc.getItems()) {
                            if (item.getNotes() != null) {
                                ...
                                passedItems += item.getNotes().stream().filter(ServiceCheckerItemNote::isPassed)
                                        .count();
