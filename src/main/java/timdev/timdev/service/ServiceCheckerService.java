...
public class ServiceCheckerService {
    ...
    public ServiceChecker create(ServiceChecker serviceChecker) {
        ...
        // if (serviceChecker.getDriver() != null && serviceChecker.getDriver().getId()
        // != null) {
        ...
        // throw new RuntimeException("A checklist already exists for this driver on " +
        // serviceChecker.getDate());
        // }
        // }
            ...
            itemNotes.forEach(in -> {
                ...
                note.setInspectionItem(inspectionItemRepo.findById(in.getItemId())
                        .orElseThrow(() -> new EntityNotFoundException(
                                "InspectionItem not found with id: " + in.getItemId())));

                note.setPassed(in.isPassed());
                note.setNote(in.getNote());
                note.setIsRequired(in.getIsRequired());
                noteRepo.save(note);
            });
        });
    ...
    public ServiceChecker updateWithInspections(Long id,
        ...
        existingChecker.setLicensePlateEstimated(updatedChecker.getLicensePlateEstimated());
        ...
        categoryItems.forEach((categoryId, itemNotes) -> {
            ...
            categoryItem.getNotes().removeIf(note -> !updatedItemIds.contains(note.getInspectionItem().getId()));
        ...
        existingChecker.getItems().removeIf(item -> !updatedCategoryIds.contains(item.getCategory().getId()));

        // 5. Save and return
        return repository.save(existingChecker);
    }

    public Page<ServiceChecker> getServiceCheckers(int page, int limit, LocalDate startDate, LocalDate endDate,
            Long driverId) {
        Pageable pageable = PageRequest.of(page - 1, limit);
        return repository.findByFilters(driverId, startDate, endDate, pageable);
    }

    public Page<ServiceChecker> getByDeviceId(int page, int limit, LocalDate startDate, LocalDate endDate,
            String deviceId) {
